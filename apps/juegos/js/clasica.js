...
function cambiarPestanaClasica(pestana) {
    ...
    ['panelAdmin','panelEmuladores','panelAgregar','panelOpciones','panelAccesorios'].forEach(id => {
    ...
    if (pestana === 'biblioteca' || pestana === 'tienda') {
        ...
        const mapa = {
            ...
            'accesorios': ['panelAccesorios', cargarPanelAccesorios, 2],
...
let accesoriosCompartidos = [];
let estadoAccesorios = {};

// ===== CARGAR ACCESORIOS =====
async function cargarAccesorios() {
    try {
        const res = await fetch('/api/accesorios');
        accesoriosCompartidos = await res.json();
        
        const resEstado = await fetch(`/api/estado-accesorios/${usuarioActual}`);
        estadoAccesorios = await resEstado.json();
    } catch (e) {
        accesoriosCompartidos = [];
        estadoAccesorios = {};
    }
}

function cargarPanelAccesorios() {
    const panel = document.getElementById('panelAccesorios');
    if (!panel) return;
    
    panel.innerHTML = `
        <div style="max-width:1200px;margin:0 auto;padding:20px;">
            <h2 style="color:#fff;">Accesorios y Consolas</h2>
            <button onclick="mostrarFormularioAccesorio()" style="padding:10px 20px;background:#e94560;color:#fff;border:none;border-radius:8px;cursor:pointer;">➕ Agregar</button>
            <div id="gridAccesorios" style="display:grid;grid-template-columns:repeat(auto-fill, minmax(250px, 1fr));gap:16px;margin-top:20px;"></div>
        </div>
    `;
    
    cargarAccesorios().then(() => mostrarAccesorios());
}

// ===== MOSTRAR ACCESORIOS =====
function mostrarAccesorios() {
    const grid = document.getElementById('gridAccesorios');
    if (!grid) return;
    
    const busqueda = document.getElementById('busquedaAccesorios')?.value?.toLowerCase() || '';
    const tipoFiltro = document.getElementById('filtroTipoAccesorio')?.value || '';
    
    let accesorios = [...accesoriosCompartidos];
    
    if (busqueda) {
        accesorios = accesorios.filter(a => a.nombre.toLowerCase().includes(busqueda));
    }
    if (tipoFiltro) {
        accesorios = accesorios.filter(a => a.tipo === tipoFiltro);
    }
    
    if (accesorios.length === 0) {
        grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:60px;color:#888;"><p style="font-size:48px;">📦</p><p>No hay accesorios</p></div>`;
        return;
    }
    
    grid.innerHTML = accesorios.map(acc => {
        const estado = estadoAccesorios[acc.id] || {};
        return `
            <div style="background:#16213e;border-radius:12px;overflow:hidden;border:1px solid #2a2a3e;">
                ${acc.imagen ? `<img src="${acc.imagen}" style="width:100%;height:180px;object-fit:cover;">` : 
                    `<div class="wrapperImagen" style="width:100%;height:180px;">
                        <img src="imagenes/${acc.nombre.toLowerCase().replace(/\s+/g, '')}.png" 
                            style="width:100%;height:100%;object-fit:cover;"
                            onerror="
                                if(this.src.endsWith('.png')) {
                                    this.src='imagenes/${acc.nombre.toLowerCase().replace(/\s+/g, '')}.jpg';
                                } else if(this.src.endsWith('.jpg')) {
                                    this.src='imagenes/${acc.nombre.toLowerCase().replace(/\s+/g, '')}.jpeg';
                                } else if(this.src.endsWith('.jpeg')) {
                                    this.src='imagenes/${acc.nombre.toLowerCase().replace(/\s+/g, '')}.webp';
                                } else {
                                    this.onerror=null;
                                    this.style.display='none';
                                    this.parentElement.innerHTML='<div style=\\'width:100%;height:180px;background:#0d0d1a;display:flex;align-items:center;justify-content:center;font-size:48px;\\'>📦</div>';
                                }
                            ">
                    </div>`
                }
                <div style="padding:16px;">
                    <h3 style="color:#fff;font-size:16px;margin-bottom:8px;">${acc.nombre}</h3>
                    <div style="display:flex;gap:8px;margin-bottom:12px;">
                        <span style="background:rgba(255,255,255,0.1);padding:4px 10px;border-radius:20px;font-size:11px;color:#ccc;">${acc.tipo}</span>
                        <span onclick="event.stopPropagation();editarPrecioAccesorio(${acc.id})" style="background:rgba(240,165,0,0.2);padding:4px 10px;border-radius:20px;font-size:11px;color:#f0a500;cursor:pointer;" title="Clic para editar">💰 ${acc.precio}€</span>
                    </div>
                    <div style="display:flex;gap:8px;">
                        <select onchange="cambiarEstadoAccesorio(${acc.id}, this.value)" style="flex:1;padding:8px;background:#1a1a2e;border:1px solid #2a2a3e;border-radius:6px;color:#fff;font-size:12px;">
                            <option value="tengo" ${estado.estado === 'tengo' ? 'selected' : ''}>✅ Lo tengo</option>
                            <option value="quiero" ${estado.estado === 'quiero' ? 'selected' : ''}>🎯 Lo quiero</option>
                            <option value="no_tengo" ${estado.estado === 'no_tengo' ? 'selected' : ''}>❌ No lo tengo</option>
                        </select>
                        ${esAdmin ? `<button onclick="eliminarAccesorio(${acc.id})" style="background:rgba(233,69,96,0.2);color:#e94560;border:none;padding:8px;border-radius:6px;cursor:pointer;">🗑️</button>` : ''}
                    </div>
                </div>
            </div>
        `;
...
// ===== CAMBIAR ESTADO ACCESORIO =====
async function cambiarEstadoAccesorio(id, estado) {
    if (!estadoAccesorios[id]) estadoAccesorios[id] = {};
    estadoAccesorios[id].estado = estado;
    
    try {
        await fetch('/api/estado-accesorios', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ usuario: usuarioActual, accesorioId: id, estado: estadoAccesorios[id] })
        });
    } catch (e) {}
}

// ===== MOSTRAR FORMULARIO ACCESORIO =====
function mostrarFormularioAccesorio() {
    const form = document.createElement('div');
    form.id = 'formAccesorioModal';
    form.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:1000;display:flex;align-items:center;justify-content:center;';
    form.innerHTML = `
        <div style="background:#1a1a2e;border-radius:16px;padding:30px;width:90%;max-width:500px;border:1px solid #2a2a3e;">
            <h3 style="color:#f0a500;margin-bottom:20px;">Nuevo Accesorio</h3>
            <div style="display:grid;gap:12px;">
                <input type="text" id="accNombre" placeholder="Nombre" style="padding:12px;background:#111;border:1px solid #2a2a3e;border-radius:8px;color:#fff;">
                <select id="accTipo" style="padding:12px;background:#111;border:1px solid #2a2a3e;border-radius:8px;color:#fff;">
                    <option value="consola">🕹️ Consola</option>
                    <option value="mando">🎮 Mando</option>
                    <option value="auriculares">🎧 Auriculares</option>
                    <option value="teclado">⌨️ Teclado</option>
                    <option value="raton">🖱️ Ratón</option>
                    <option value="monitor">🖥️ Monitor</option>
                    <option value="otro">📦 Otro</option>
                </select>
                <input type="number" id="accPrecio" placeholder="Precio (€)" step="0.01" style="padding:12px;background:#111;border:1px solid #2a2a3e;border-radius:8px;color:#fff;">
                <input type="file" id="accImagen" accept="image/*" style="color:#fff;">
            </div>
            <div style="display:flex;gap:10px;margin-top:20px;">
                <button onclick="guardarAccesorio()" style="flex:1;padding:14px;background:#4ecca3;color:#000;border:none;border-radius:8px;cursor:pointer;font-weight:700;">💾 Guardar</button>
                <button onclick="document.getElementById('formAccesorioModal').remove()" style="padding:14px 20px;background:transparent;color:#888;border:1px solid #2a2a3e;border-radius:8px;cursor:pointer;">Cancelar</button>
            </div>
        </div>
    `;
...
async function guardarAccesorio() {
    const imagenFile = document.getElementById('accImagen').files[0];
    let imagenData = null;
    ...
    const accesorio = {
        ...
        tipo: document.getElementById('accTipo').value,
    ...
    try {
        await fetch('/api/accesorios', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ usuario: usuarioActual, accesorio, esEdicion: false })
        });
        
        document.getElementById('formAccesorioModal').remove();
        cargarPanelAccesorios();
    } catch (e) {
...
// ===== EDITAR PRECIO ACCESORIO =====
async function editarPrecioAccesorio(id) {
    const acc = accesoriosCompartidos.find(a => a.id === id);
    if (!acc) return;
    
    const nuevoPrecio = prompt('💰 Nuevo precio para ' + acc.nombre + ':', acc.precio || '');
    
    if (nuevoPrecio !== null) {
        const precioNum = parseFloat(nuevoPrecio);
        if (!isNaN(precioNum) && precioNum >= 0) {
            acc.precio = precioNum;
        } else if (nuevoPrecio === '') {
            acc.precio = 0;
        }
        ...
        try {
            await fetch('/api/accesorios', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ usuario: usuarioActual, accesorio: acc, esEdicion: true })
            });
            mostrarAccesorios();
        } catch (e) {
